...
\subsection{Additional model description}
Any input x goes first through four linear layers followed by two interpolation functions, one for backcast: $g_\theta^b$ and the other for forecast: $g_\theta^f$.
...
\small
$g_\theta = \sum_{i=0}^p \theta_i t^i$ where $\vect{t} = [0,1,2,\cdots,H-2, H-1]^T/H$. The trend forecast has the form: 
\[
	\hat{\vect{y}}_{i,j}^{tr} = \matr{T} \theta_{i,j}
\]
One important point to make is that the weight could be shared among the stacks.
